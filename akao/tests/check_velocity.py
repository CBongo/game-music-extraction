#!/usr/bin/env python3
"""Quick script to check velocity distribution in a MIDI file."""

import sys
import struct

if len(sys.argv) < 2:
    print("Usage: python3 check_velocity.py <midi_file>")
    sys.exit(1)
...
clipped = sum(1 for v in velocities if v == 127)
...
print(f"Clipped at 127: {clipped} ({clipped_pct:.1f}%)")
