#!/usr/bin/perl
#
# bionic_mid.pl - bionic.nes to midi generator
# chris bongaarts 24 april 2025
#
...
#sound stuff is in PRG bank C
seek NES, 0x4000 * 0xC, 1;  # whence = 1 means relative to currentn pos
read NES, $buf, 0x4000;
close NES;
...
@track_offsets = &$readsong("v*", 0x8a50, 0x40 * 2);

# up to 0f and 40-44 are bgm, rest are sfx
for (my $song=0; $song < 0x45; $song++) {
  $song = 0x40 if $song == 0x10;  # skip sfx
...
sub parse_track {
  ...
  while (!$done) {
	...
	$done = 1;
      ...
      # std durations (actual addr in bionic is 8975/897d for these)
